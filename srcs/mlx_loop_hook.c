...
static void	init_step_and_side_dist(t_data *data, t_vec3 step, t_vec3 side_dist)
{
	...
	{
		step[0] = 1.0;
		side_dist[0] = ((int)(data->actual_ray_pos[0]) + 1.0 - data->actual_ray_pos[0]) * data->actual_delta_dist[0];
	}
	...
	{
		step[1] = 1.0;
		side_dist[1] = ((int)(data->actual_ray_pos[1]) + 1.0 - data->actual_ray_pos[1]) * data->actual_delta_dist[1];
	}
}
...
static void	get_hit(t_data *data, t_vec3 ray_pos, t_vec3 ray_dir, t_hit_info *ret)
{
	...
	printf("Wall detected at %f %f\n", map_pos[0], map_pos[1]);
	ret->side = side;
	ft_vec3_copy(ret->collision_pos, map_pos);

	if (side == 0)
		ret->corrected_dist = (map_pos[0] - ray_pos[0] + (1 - data->actual_step[0]) / 2) / ray_dir[0];
	else
		ret->corrected_dist = (map_pos[1] - ray_pos[1] + (1 - data->actual_step[1]) / 2) / ray_dir[1];
}
